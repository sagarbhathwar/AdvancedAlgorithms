...
public:
    ...
    static Matrix<T> strassen_multithreaded(const Matrix<T>& A, const Matrix<T>& B);
...
template <typename T>
Matrix<T> Matrix<T>::strassen_multithreaded(const Matrix<T>& A, const Matrix<T>& B)
{
    int n = A.n_rows_;
    ...
    else
    {
        ...
        g.run([&]{m1 = strassen_multithreaded(a00 + a11, b00 + b11);});
        g.run([&]{m2 = strassen_multithreaded(a10 + a11, b00);});
        g.run([&]{m3 = strassen_multithreaded(a00, b01 - b11);});
        g.run([&]{m4 = strassen_multithreaded(a11, b10 - b00);});
        g.run([&]{m5 = strassen_multithreaded(a00 + a01, b11);});
        g.run([&]{m6 = strassen_multithreaded(a10 - a00, b00 + b01);});
        g.run([&]{m7 = strassen_multithreaded(a01 - a11, b10 + b11);});
        g.wait();
...
int main()
{
    ...
    auto C1 = Matrix<float>::strassen_multithreaded(A, B);
